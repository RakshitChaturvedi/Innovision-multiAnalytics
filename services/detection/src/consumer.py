...
FIXES APPLIED
-------------
1. Imports pointed at `shared.schemas.contracts` and
   `shared.storage.minio_client`; neither module exists.
2. Blocking work (JPEG decode, YOLO inference) ran directly on the
   event loop, stalling Redis reads and heartbeats for the whole
   inference duration. Now offloaded with asyncio.to_thread.
3. A new YOLODetector was constructed for every batch.
4. Messages were acked as soon as the frame entered the batch, so a
   crash between batching and publishing lost frames silently. ACK is
   now deferred until after publish (manual_ack).
5. Detections were inserted one INSERT per track; now a single
   executemany, with dict params JSON-encoded (asyncpg cannot adapt a
   raw dict to jsonb).
6. Publisher/shake subscriber each opened their own Redis connection
   with no pooling and no close on failure.
7. The shake channel was hardcoded to the test camera.
"""
...
from shared.storage.storage_minio_client import get_storage
...
class DetectionConsumer(BaseStreamConsumer):
    ...
    def __init__(self) -> None:
        ...
        self._shake_subscriber_task: asyncio.Task | None = None
    ...
    async def start(self) -> None:
        ...
        self._shake_subscriber_task = asyncio.create_task(
            self._shake_subscriber(),
            name="detection-shake-subscriber",
        )
    ...
    async def _fetch_frame(self, frame_event: FrameEvent) -> bytes:
        """
        Redis cache first, object storage as fallback.
        """

        if frame_event.frame_provider in (
            FrameProvider.CACHE,
            FrameProvider.REDIS,
        ):
            try:
                ...
                logger.warning(
                    "cache_miss_falling_back_to_object_store "
                    "reference=%s",
                    frame_event.frame_reference,
                )
                ...
                logger.warning(
                    "cache_fetch_error reference=%s error=%s",
                    frame_event.frame_reference,
                    exc,
                )
    ...
    async def _shake_subscriber(self) -> None:
        """
        Subscribe to camera-shake notifications.

        Uses a pattern subscription so cameras added at runtime are
        picked up without a restart.
        """

        assert self._side_redis is not None

        pubsub = self._side_redis.pubsub()

        pattern = "camera:shake:*"

        await pubsub.psubscribe(pattern)

        logger.info("shake_subscriber_ready pattern=%s", pattern)
        ...
        try:
            async for message in pubsub.listen():
                ...
                logger.warning(
                    "camera_shake_received camera_id=%s", camera_id
                )

                self._tracker_manager.on_shake(camera_id)
